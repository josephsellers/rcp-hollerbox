...
class Mplus_HollerBox {
  ...
  public function mplus_maybe_display_items() {
    ...
    foreach (self::$active as $key => $value) {
      ...
      continue;
      if(rcp_user_can_access( get_current_user_id( ), $value ) && admin_check($value)){
	...
	?>
			<p>
				<label for="rcp-hide-in-admin">
					...
					<?php _e( 'Hide this content from Admin.', 'mplus-holler' ); ?>
				</label>
			</p>
